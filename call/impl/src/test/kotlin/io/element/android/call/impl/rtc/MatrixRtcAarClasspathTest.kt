...
/**
 * Guards the way `rtc/local` exposes `matrixrtc-release.aar`: a bare Gradle project publishing the
...
class MatrixRtcAarClasspathTest {
     ...
     * than here. `docs/FEEDBACK.md` records that upstream has not confirmed the jar is meant to be
     ...
     * so the texture view in `call/ui` builds the equivalent out of [livekit.org.webrtc.EglRenderer]
     * directly. That only works because `EglRenderer` can take a `SurfaceTexture`, which is a
     * narrower thing to depend on than a whole renderer and is worth asserting by itself: losing
     * this overload means falling back to `SurfaceViewRenderer` and losing every tile animation.
     */
    @Test
